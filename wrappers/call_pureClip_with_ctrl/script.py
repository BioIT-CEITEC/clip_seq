#########################################
# wrapper for rule: call_pureClip_with_ctrl
#########################################
import subprocess
import os
...
try:
    ...
    command = "$(which time) pureclip -i "+snakemake.input.bam+" -bai "+snakemake.input.bai+\
              " -ibam "+snakemake.input.cbam+" -ibai "+snakemake.input.cbai+\
              " -g "+snakemake.input.gen+" -nt "+str(snakemake.threads)+" -iv '1;2;3;' -o "+snakemake.output.bed+" -or "+snakemake.output.bed2+" >> "+log_filename+" 2>&1"
    f = open(log_filename, 'at')
