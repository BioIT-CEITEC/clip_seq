...
# command = "CLAM permutation_callpeak -i "+snakemake.params.uniq_bam+" "+snakemake.params.mult_bam+" -o "+snakemake.params.dir+" --gtf "+snakemake.input.gtf[0]+" -p "+str(snakemake.threads)+" --qval-cutoff "+str(snakemake.params.qval_cutoff)+" --merge-size "+str(snakemake.params.merge_size)+" --extend "+str(snakemake.params.extend_peak)+" --random-state 777 "+strand+" >> "+snakemake.log.run+" 2>&1"
# f = open(snakemake.log.run, 'at')
# f.write("## COMMAND: "+command+"\n")
# f.close()
# shell(command)
