...
if os.path.isfile(snakemake.input.bed) and os.path.getsize(snakemake.input.bed) > 0:
    command = "Rscript "+snakemake.params.rscript+" "+snakemake.input.bed+" "+snakemake.input.gtf+" "+snakemake.output.bed+" "+snakemake.params.feat_type+" "+snakemake.params.annotate_by+" >> "+log_filename+" 2>&1"
    f = open(log_filename, 'at')
